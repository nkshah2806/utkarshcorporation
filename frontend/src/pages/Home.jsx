...
export default function Home() {
  ...
  useEffect(() => {
    ...
    api.get("/products?bestseller=true&limit=4").then((r) => setBest(r.data));
          ...
          {categories.slice(0, 6).map((c, idx) => (
            ...
            >
              ...
              <div className="relative h-full min-h-[180px] p-5 flex items-end">
                <div>
                  <div className="font-serif-display text-xl lg:text-2xl text-[#F9F6F0] mb-1">{c.name}</div>
                  <div className="text-xs text-[#F9F6F0]/70 line-clamp-2">{c.description}</div>
                </div>
              </div>
            </Link>
          ))}
