    ...
    <div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-36 lg:py-44">
          <div className="max-w-2xl text-[#F9F6F0]">
            <div className="inline-flex items-center gap-2 bg-[#C5A059]/20 backdrop-blur-sm border border-[#C5A059]/40 rounded-full px-4 py-1.5 text-xs uppercase tracking-[0.2em] text-[#F9F6F0] mb-6">
              <Leaf className="w-3.5 h-3.5" /> <LocalizedText value={hero.badge} />
            </div>
            <h1 className="font-serif-display text-4xl sm:text-5xl lg:text-6xl leading-[1.05] mb-6">
              <LocalizedText value={hero.titleLine1} /><br />
              <span className="italic text-[#C5A059]"><LocalizedText value={hero.titleLine2} /></span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                <LocalizedText value={hero.primaryCtaText} fallback={t("home.shopNow")} /> <ArrowRight className="w-4 h-4" />
              </Link>
              <Link
              ...
              >
                <LocalizedText value={hero.secondaryCtaText} fallback={t("home.learnMore")} />
              </Link>
            </div>
          </div>
        </div>
            ...
            {trustBadges.map((badge) => {
              ...
              return (
                <div key={badge.id || badge.text} className="flex items-center gap-3">
                  <IconComponent className="w-5 h-5 text-[#C5A059]" /> <LocalizedText value={badge.text} />
                </div>
              );
            })}
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
        <div className="flex items-end justify-between mb-10 gap-4">
          <div>
            <div className="text-xs uppercase tracking-[0.2em] text-[#5C4033] mb-3">{t("home.shopByConcern")}</div>
            <h2 className="font-serif-display text-3xl sm:text-4xl lg:text-5xl text-[#1A3626]">
              {t("home.rootedRemedies")}
            </h2>
          </div>
          ...
          {categories.slice(0, 6).map((c, idx) => (
            ...
            >
              ...
              <div className="relative h-full min-h-[180px] p-5 flex items-end">
                <div>
                  <div className="font-serif-display text-xl lg:text-2xl text-[#F9F6F0] mb-1 font-bold drop-shadow-sm"><LocalizedText value={c.name} /></div>
                  <div className="text-xs text-[#F9F6F0]/90 line-clamp-2 font-medium"><LocalizedText value={c.description} /></div>
                </div>
              </div>
            </Link>
          ))}
      ...
      <section className="bg-white py-20 lg:py-28 border-y border-[#1A3626]/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="text-xs uppercase tracking-[0.2em] text-[#5C4033] mb-3">{t("home.handpickedForYou")}</div>
            <h2 className="font-serif-display text-3xl sm:text-4xl lg:text-5xl text-[#1A3626]">
              {t("home.featuredThisSeason")}
            </h2>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div>
            <div className="text-xs uppercase tracking-[0.2em] text-[#5C4033] mb-3"><LocalizedText value={mission.badge} fallback={t("home.ourMission")} /></div>
            ...
            <div className="grid grid-cols-3 gap-6">
              {mission.stats?.map((stat, idx) => (
                <div key={stat.id || idx}>
                  ...
                  <div className="text-xs text-[#5C4033] uppercase tracking-wider"><LocalizedText value={stat.label} /></div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="flex items-end justify-between mb-10 gap-4">
              <div>
                <div className="text-xs uppercase tracking-[0.2em] text-[#C5A059] mb-3 flex items-center gap-2">
                  <Sparkles className="w-3.5 h-3.5" /> {t("home.lovedByCommunity")}
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
        <div className="text-center mb-12">
          <div className="text-xs uppercase tracking-[0.2em] text-[#5C4033] mb-3">{t("home.voicesFromFamilies")}</div>
          <h2 className="font-serif-display text-3xl sm:text-4xl lg:text-5xl text-[#1A3626]">{t("home.whatCustomersSay")}</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((item, i) => (
            <div key={item.id || i} data-testid={TID.testimonial} className="bg-white rounded-2xl p-8 border border-[#1A3626]/10 relative">
              <Quote className="w-8 h-8 text-[#C5A059]/40 mb-4" />
              <p className="text-[#1A3626]/85 leading-relaxed mb-6">
                <span aria-hidden="true">"</span>
                <span><LocalizedText value={item.body} /></span>
                <span aria-hidden="true">"</span>
              </p>
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold text-[#1A3626]"><LocalizedText value={item.name} /></div>
                <div className="flex gap-0.5">
                  {Array.from({ length: item.stars || 5 }).map((_, k) => (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 lg:pb-28">
        <div className="relative rounded-2xl overflow-hidden bg-[#5C4033] p-10 md:p-16 text-[#F9F6F0]">
          <div className="max-w-2xl">
            ...
            >
              <LocalizedText value={distributorCta.ctaText} fallback={t("home.learnMore")} /> <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </div>
      </section>
